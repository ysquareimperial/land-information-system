          ...
          <tbody>
            {
              data[0]?.map((item) => (
                <tr >
                  <td >{item.status === 'application' ? <button className='pending'>Pending</button> : item.status === 'file_no_generated' ? <button className='paid'>Paid</button> : item.status === 'application-fee-paid' ? <button className='approved'>Approved</button>  : ''}</td>
