    ...
    <div>
        ...
        <Row className="mb-1">
           <Col md={6}>
            ...
            <div>
              <select className="input_field" name='type' value={list.type} onChange={handleChange}>
                  ...
                  <tbody>
                    {
                      data[0]?.map((item)=>(
                        <tr>
                          ...
                          <td><button className='app_btn' onClick={()=>{setList((p)=>({...p,application_id:item.application_id,applicant_full_name:item.Applicant_full_name}));toggle3()}}>select</button></td>
                        </tr>
                      ))
                    }
                  </tbody>
          ...
          <div className='d-flex justify-content-around mt-4'>
            {
              byResidential?.map(item=><span style={{fontSize:10}}> <Input checked type="checkbox" />{item.name} :{parseInt(item.amount).toLocaleString()}</span>)
            }
          </div>
          </Col>
        </Row>
